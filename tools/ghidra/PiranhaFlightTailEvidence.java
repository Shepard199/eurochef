...
public class PiranhaFlightTailEvidence extends GhidraScript {
 @Override public void run() throws Exception {
  Address s=toAddr(0x00467990L); disassemble(s);
  InstructionIterator it=currentProgram.getListing().getInstructions(s,true);
  int n=0;
  while(it.hasNext()&&n<210){
   Instruction ins=it.next();
   if(ins.getAddress().getOffset()>=0x00467B70L) break;
